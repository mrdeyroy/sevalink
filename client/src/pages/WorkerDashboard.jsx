...
const WorkerDashboard = () => {
    ...
    const updateStatus = async (id, status) => {
        setUpdatingId(id);
        try {
            await axios.put(`https://sevalink-zygf.vercel.app/api/requests/${id}/status`, { status }, config);
    ...
    const handleProofUploadAndResolve = async (file) => {
        ...
        try {
            ...
            await axios.put(
                `https://sevalink-zygf.vercel.app/api/requests/${proofTask._id}/status`,
                formData,
                {
                    headers: {
                                    ...
                                    >
                                        ...
                                        <div className="px-5 py-3 flex-grow">
                                            ...
                                            <div className="rounded-xl overflow-hidden h-36 bg-gray-100 mb-1 relative group border border-gray-100">
                                                <img
                                                    src={(!task.imageUrl || task.imageUrl.startsWith("http")) ? "/citizen_issue.png" : `https://sevalink-zygf.vercel.app${task.imageUrl}`}
                                                    alt="Reported issue"
